...
const windowAorE =document.getElementById()
...
function onSubmit(event) {
  ...
  if (!selectedAudioType) {
    messageError.textContent  = "¡Error! Por favor, seleccione un tipo de audio.";
    popup.style.display = 'flex';
    okButton.style.display = 'block';
    return;
  }
...
function validateTitle(inputElement) {
  ...
  if (!/^[a-zA-Z\s]+$/.test(inputValue)) {
    messageError.textContent  = `¡Error! \n No se admiten caracteres especiales en: ${placeholderText}`;
    popup.style.display = 'flex';
    okButton.style.display = 'block';
    return;
    // inputElement.value = "";

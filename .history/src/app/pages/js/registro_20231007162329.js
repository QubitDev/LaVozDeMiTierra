...
function onSubmit(event) {
  ...
  const campos = [
    { valor: titulo, nombre: "Título de audio", longitudMinima: 10 },
    { valor: procedencia, nombre: "Procedencia", longitudMinima: 4 },
    { valor: narrador, nombre: "Narrador", longitudMinima: 4 },
    { valor: musica, nombre: "Música de fondo", longitudMinima: 10 },
  ];

  for (const campo of campos) {
    if (campo.valor.length < campo.longitudMinima) {
      alert(`El campo '${campo.nombre}' debe tener al menos ${campo.longitudMinima} caracteres.`);
      return;
    }
  }

  // validar que este seleccionado un tipo deaudio

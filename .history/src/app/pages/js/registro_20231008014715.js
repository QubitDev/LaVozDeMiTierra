...
function onSubmit(event) {
  ...
  if (!tipoAudioSeleccionado) {
    messageError.textContent  = "¡Por favor, seleccione un tipo de audio!";
    popup.style.display = 'flex';
    okButton.style.display = 'block';
    return;
  }
  ...
  const fields = [
    { valor: titulo, nombre: "Título de audio", minimumLength: 4, elementoError: "error_titulo_audio" },
    { valor: musica, nombre: "Música de fondo", minimumLength: 4, elementoError: "error_musica_fondo" },
    { valor: procedencia, nombre: "Procedencia", minimumLength: 4, elementoError: "error_procedencia" },
    { valor: narrador, nombre: "Narrador", minimumLength: 4, elementoError: "error_narrador" },
  ];

  for (const field of fields) {
    const messageError = document.getElementById(field.elementoError);

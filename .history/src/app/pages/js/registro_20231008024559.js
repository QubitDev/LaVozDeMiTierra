...
function onSubmit(event) {
  ...
  if (!titulo || !musica || !procedencia || !formato || !narrador ||!duracion|| !audio || !text) {
    messagePopup.textContent = `❌¡Error! Faltan Datos.`;
  ...
  // cargar el mensage si todo sale bien.
  messagePopup.textContent = `✅¡Se subio correctamente el audio!🎉`;
  popup.style.display = 'flex';
  messagePopup.style.whiteSpace = 'pre-line'; 
  windowAorE.style.width = '400px';
...
function validateTitle(inputElement) {
  ...
  if (!/^[a-zA-Z\s]+$/.test(inputValue)) {
    messagePopup.textContent  = `❌¡Error!<br>No se admiten caracteres especiales en: ${placeholderText}`;
    messagePopup.style.whiteSpace = 'pre-line'; 
    popup.style.display = 'flex';
    windowAorE.style.width = '400px';
    okButton.style.display = 'block';
...
audioInput.addEventListener("change", function () {
  const selectedAudioFile = thaudioInput.files[0];

...
function onSubmit(event) {
  ...
  if (titulo.length < 10 || procedencia.length < 4 || narrador.length < 4 || musica.length < 10) {
    alert("El campo 'Título de audio' debe tener al menos 10 caracteres.");
    return;
  }

  if (procedencia.length < 4) {
    alert("El campo 'Procedencia' debe tener al menos 4 caracteres.");
    return;
  }

  if (narrador.length < 4) {
    alert("El campo 'Narrador' debe tener al menos 4 caracteres.");
    return;
  }

  if (musica.length < 10) {
    alert("El campo 'Música de fondo' debe tener al menos 10 caracteres.");
    return;
  }

  if (!titulo || !musica || !procedencia || !formato || !narrador || !tipoAudioSeleccionado || !audio || !text) {

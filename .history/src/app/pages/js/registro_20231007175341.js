...
function onSubmit(event) {
  ...
  const campos = [
    { valor: titulo, nombre: "Título de audio", longitudMinima: 10, elementoError: "error_titulo_audio" },
    { valor: musica, nombre: "Música de fondo", longitudMinima: 10, elementoError: "error_musica_fondo" },
   ...
   if (!titulo || !musica || !procedencia || !formato || !narrador || !audio || !text) {
    mensajeFaltaDatos.textContent = "¡Error! Faltan Datos.";
    enviarDatos = false; // Establece la variable de bandera en false en caso de error
  } else {
    mensajeFaltaDatos.textContent = ""; // Borra el mensaje de error si no hay errores
  }
  if (!enviarDatos) {
    return;
  }
  ...
  if (!tipoAudioSeleccionado) {
    const mensajeErrorTipoAudio = document.getElementById("error_tipo_audio");

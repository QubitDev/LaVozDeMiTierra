...
function onSubmit(event) {
  ...
  if (!titulo || !musica || !procedencia || !formato || !narrador ||!duracion|| !audio || !text) {
    messageErrors(`❌¡Error! Faltan Datos.`,350);
    return;
  }
...
function message(messageX, length){
  ...
  message.includes("❌")v? this.okButton.style.display = 'block';
  : this.verifyButton.style.display = 'block';
}
// cargar el mensage si todo sale bien.
function messageCongratulations(){
  this.messagePopup.textContent = `✅¡Se subio correctamente el audio!🎉`;
  this.popup.style.display = 'flex';
  this.messagePopup.style.whiteSpace = 'pre-line'; 

...
function onSubmit(event) {
  ...
  let tipoAudioSeleccionado = false;
  let tipoAudio = "";
  ...
  if (!tipoAudioSeleccionado) {
    const mensajeErrorTipoAudio = document.getElementById("message");

    mensajeErrorTipoAudio.textContent = "Por favor, seleccione un tipo de audio.";
...
// Código para redireccionar al reproductor cuando se haga clic en el botón "Verificar"
// Función para manejar el botón "Verificar"
function handleVerifyButton() {
  const mensajeError = document.querySelector(".message").textContent;

  if (mensajeError === "¡Error! Faltan Datos.") {
    // Si es un mensaje de error o falta de información, mostrar "OK" y cerrar el popup
    alert("OK");
    const popup = document.getElementById("popup");
    popup.style.display = "none";
  } else {
    // Si no hay errores, redireccionar a otra página
    window.location.href = "url_de_destino.html"; // Reemplaza 'url_de_destino.html' con la URL real de destino
  }
}

// Agregar un event listener al botón "Verificar"

...
document
  .getElementById("textFileInput")
  .addEventListener("change", function () {
    const textFile = this.files[0]; // Obtén el archivo de texto seleccionado

    if (textFile) {
      const reader = new FileReader();

      reader.onload = function (event) {
        const content = event.target.result; // Contenido del archivo de texto
        console.log("Contenido del archivo de texto:", content);
      };

      reader.readAsText(textFile); // Lee el contenido del archivo como texto
    }
  });
